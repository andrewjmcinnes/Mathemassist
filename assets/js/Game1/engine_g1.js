...
//Variables
var canvas, context, LARGURA, ALTURA, img, GameStatus = 0, record, GRAVITY = 1.5, life = 3, score = 0, aux=0, sound = false, frames = 0,
    ...
    insere: function(){
	    if(aux == 0 && (this._obs.length > Math.floor(3 * Math.random()))){
	    	...
	    	this._obs.push({
	            ...
	            x: 300 + Math.floor(571 * Math.random()),
	            y: 0,
	            name: Answers[Math.floor(Answers.length * Math.random())].name,
	            simbol: Answers[Math.floor(Answers.length * Math.random())].id
	        });   
        } 	       
...
function elementSort(){
    var rand = Math.floor(Answers.length * Math.random());
    quiz.name = Answers[rand].name;
    quiz.simbol = Answers[rand].id;
}
...
function play(){
    ...
    if(GameStatus == Status.start) {
        ...
        canvas.onclick = function(event){
            ...
            GRAVITY = 1.5;
            elementSort();
            alert("Can you help Sammy to create her rocket fuel to get home by selecting the correct chemicals? You must select the answer to the problem that is presented by Sammy the cat. You have three lives, and if you miss a answer or get the question wrong, you lose one life. For each correct answer, you get one point!")
...
function main(){
	...
	img = new Image();
	img.src = "http://localhost/mathemassist/assets/images/Game1/images.png";
